...
%% ----------------------------
%  Input Data (CW pattern)
%% ----------------------------
data_in = ones(1, 32);   % Continuous-wave: all 1's

% Interleaver positions (1-based)
...
% Use custom upsample to avoid errors in Octave
tx_shaped = conv(upsample_custom(bpsk_symbols, sps), rrcFilter, 'same');

%% ----------------------------
%  Spectrum Visualization
%% ----------------------------
fs = 7200;             % coded bit rate
N = 8192;
spec = abs(fftshift(fft(tx_shaped, N)));
spec = spec / max(spec);
...
ylabel('Magnitude (dB)');
title('Spectrum of BPSK Modulated Turbo-Coded (Rate 1/3) Signal (CW Bits)');
